...
const initializeSampleApiKeys = () => {
  // Sample API key: cf_live_1234567890abcdef
  apiKeyStore.set('cf_live_1234567890abcdef', {
    userId: 'user_001',
    permissions: ['read:assets', 'read:market', 'read:portfolio'],
  ...
  // Sample API key: cf_live_0987654321fedcba
  apiKeyStore.set('cf_live_0987654321fedcba', {
    userId: 'user_002',
    permissions: ['read:assets', 'read:market', 'read:portfolio', 'write:trading', 'write:portfolio'],
    createdAt: new Date('2024-01-01'),
    lastUsed: new Date(),
    isActive: true,
    rateLimit: 1000
  ...
  // Sample API key: cf_live_admin_superuser
  apiKeyStore.set('cf_live_admin_superuser', {
    userId: 'admin_001',
...
export const createApiKey = async (userId: string, permissions: string[]): Promise<string> => {
  try {
    // Generate a new API key
    const apiKey = `cf_live_${Date.now()}_${Math.random().toString(36).substring(2, 15)}`;
...
export const getApiKeyInfo = async (apiKey: string): Promise<ApiKeyInfo | null> => {
  try {
